...
export class UsuariosService {
  ...
  constructor(
    ...
    @InjectRepository(AvatarUsuarios) private readonly avatarRepository: Repository<AvatarUsuarios>,
  ) {}
  ...
  async create(createUsuarioDto: CreateUsuarioDto): Promise<Usuario> {
    ...
    //Le sumo 1
    const siguienteid = ultimousuario.length > 0 ? ultimousuario[0].idUsuario + 1 : 1;
    ...
    const siguienteIdAvatar = ultimoAvatar.length > 0 ? ultimoAvatar[0].idImagenAvatar + 1 : 1;
...
async findOne(rut: string): Promise<Usuario> {
  const usuario = await this.usuarioRepository.findOne({ where: { rut: rut } });
  if (!usuario) {
